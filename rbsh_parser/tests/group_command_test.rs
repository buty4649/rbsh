...
fn test_group_command() {
    assert_parse!("{ foo; bar; baz; }" => Ok(vec![group_command!(
        body: vec![
    ...
    assert_parse!("{ { foo; }; bar; { baz; }; }" => Ok(vec![group_command!(
        body: vec![
            ...
            command!(name: vec![bare!(bar)]),
            group_command!(body: vec![command!(name: vec![bare!(baz)])]),
        ]
    )]));
    assert_parse!(indoc!("
            {
    ...
    assert_parse!("{ (foo); bar; }" => Ok(vec![group_command!(
        body: vec![
    ...
    assert_parse!("{ foo() { bar; }; foo; }" => Ok(vec![group_command!(
        body: vec![
...
#[test]
fn test_group_command_with_redirect() {
    assert_parse!("{ foo; bar; baz; } > hoge" => Ok(vec![group_command!(
        body: vec![
            command!(name: vec![bare!(foo)]),
            command!(name: vec![bare!(bar)]),
            command!(name: vec![bare!(baz)]),
