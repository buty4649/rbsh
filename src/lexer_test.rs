...
mod test {
    ...
    fn test_lex_backquote() {
        ...
        assert_lex!(lex_backquote, "`foobar", Err(ParseError::eof(loc!(8, 1))));
        assert_lex!(lex_backquote, "`", Err(ParseError::eof(loc!(2, 1))));
    }
    ...
    fn test_lex_parameter() {
        ...
        assert_lex!(lex_parameter, "${foobar", Err(ParseError::eof(loc![9, 1])));
    }
    ...
    fn test_lex_command_substitute() {
        ...
        assert_lex!(
            lex_command_substitute,
            "$(foobar",
            Err(ParseError::eof(loc!(9, 1)))
        );
        assert_lex!(
            lex_command_substitute,
            "$(foobar$(bar)",
            Err(ParseError::eof(loc!(15, 1)))
        );
    }
